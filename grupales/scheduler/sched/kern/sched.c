...
cprintf_stats(void)
{
	cprintf("[STATS] Scheduler calls: %d\n", scheduler_calls);
	cprintf("[STATS] Times no runnable envs were found: %d\n",
	        no_runnable_envs_found);
	for (int i = 0; i < NENV; i++) {
		struct Env e = envs[i];
		if (e.env_id != 0)
			cprintf("[STATS] Times the env with id %d was run: "
			        "%d\n",
			        e.env_id,
			        e.env_runs);
	}
...
#ifdef USE_PRIORITY_SCHEDULER  // Priority Scheduler
	int maxPriority = -1;
