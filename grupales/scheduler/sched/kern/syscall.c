...
static int
sys_env_get_priority(envid_t envid, int *priority)
{
	struct Env *env;
	int resp = envid2env(envid, &env, 0);
	if (resp)
		return resp;  // Access if statement when 0 is not returned.
	*priority = env->env_priority;
	return 0;
}
...
sys_env_set_priority(envid_t envid, int priority)
{
	...
	if (resp)
		return resp;  // Access if statement when 0 is not returned.
	if (env->env_id == curenv->env_id && env->env_priority < priority)
		return 0;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_get_priority:
		return sys_env_get_priority(a1, (int *) a2);
	case SYS_yield:
		sys_yield();  // No return
	default:
		return -E_INVAL;
